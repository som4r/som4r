...
def detect_and_draw( img ):
    ...
    if( cascade ):
        ...
        params = "<?xml version='1.0' encoding='UTF-8'?><VeiculoRO><direcao>5</direcao><velocidade>0</velocidade><status>0</status></VeiculoRO>"
        if faces:
            for r in faces:
                ...
                # tamanho considerado.
		tam = max(r.width, r.height);
		# tamanho padrao.
		esperado = 52;
		if (tam < (esperado*0.9)): # p/frente
		    print "ahead"
                    params = '''<VeiculoRO><direcao>2</direcao>
                        <velocidade>1</velocidade></VeiculoRO>'''
		elif (tam > (esperado*1.1)): # p/tras
		    print "back off"
                    params = '''<VeiculoRO><direcao>8</direcao>
                        <velocidade>1</velocidade></VeiculoRO>'''
		else:
		    print "Ok. Dont move."
        ...
        conn = httplib.HTTPConnection("192.168.1.107:8080")
        conn.request("POST", "/ws_veiculo/resources/veiculo", params)
