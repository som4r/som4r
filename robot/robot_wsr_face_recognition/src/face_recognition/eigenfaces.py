...
#    def findmatchingimage(self,imagename,selectedfacesnum,thresholdvalue):
    def findmatchingimage(self,imagename,thresholdvalue):
        ...
        usub=self.bundle.eigenfaces[:self.eigenfaces_calc,:]
    ...
    def doCalculations(self,dir,imglist):#,selectednumeigenfaces):
        self.eigenfaces_calc=self.createFaceBundle(imglist);
        ...
        f2.close()
        #write to cache
        cachefile=join(dir,"saveddata.egfnum")
        f3=open(cachefile,"w")
        pickle.dump(self.eigenfaces_calc,f3)
        f3.close()
...
#        usub=eigenfaces[:selectedfacesnum,:]
        usub=eigenfaces[:self.eigenfaces_calc,:]
    ...
    def createFaceBundle(self,imglist):                
        ...
        # calculando nro de autovetores q representam 90% dos autovalores.
        eigenfaces_num = int(numimgs/2) # valor padrao
        acum = 0
        soma = evals.sum()
#        print "========================"
#        print "nro de autovalores = " + str(len(evals))
        for i in range(len(evals)):
            acum = acum + evals[i]
#            print str(i+1) + " = " + str(int(acum*100/soma)) + "%"
            if acum >= (soma*0.9):
                eigenfaces_num = i+1
                break
#        print "eigenvalues_limit = " + str(self.eigenvalues_limit)
#        print "========================"
...
#    def checkCache(self,dir,ext,imglist,selectedfacesnum,thrval):
    def checkCache(self,dir,ext,imglist,thrval):
        cachefile=join(dir,"saveddata.cache")
        cachefile3=join(dir,"saveddata.egfnum")
        cache_changed=True
        try:
            f=open(cachefile)
            f3=open(cachefile3)
            ...
            self.eigenfaces_calc=pickle.load(f3)
